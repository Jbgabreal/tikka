...
const Chat = () => {
  ...
  const handleSendMessage = useCallback(async ({ input, attachments }: { input: string; attachments: Attachment[] }) => {
    ...
    if (currentStep) {
      ...
      setIsGenerating(true);
      try {
        // Always include walletAddress in context for multi-step flows
        const payload = {
          messages,
          currentStep,
        ...
        // Unified unsigned transaction handling for both swap and token creation
    ...
    setIsGenerating(true);
    try {
      // Always include walletAddress if available
      const payload = {
        messages,
        walletAddress: publicKey?.toBase58() || undefined,
      };
